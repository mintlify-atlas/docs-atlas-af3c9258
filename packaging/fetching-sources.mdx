---
title: "Fetching and Verifying Package Sources in IX Packages"
sidebarTitle: "Fetching Sources"
description: "IX supports tarball downloads, git checkouts, and multiple fetch URLs. Every source is SHA-256 verified. Learn the four hash types used in ix.sh recipes."
---
...
IX downloads `<git_repo>/archive/refs/tags/<git_branch>.tar.gz` and builds a vendored `.pzd` archive from it. The `git_sha` is the SHA-256 of that `.pzd` archive — **not** a git commit hash. See [The Four Hash Types](#the-four-hash-types) below for the distinction.
